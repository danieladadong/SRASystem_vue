<template>
  <el-dialog
    ...
    <el-form
      ...
      <el-form-item v-if="isadd" label="审核状态">
        <el-input
            :disabled="isadd"
...
<script>
import {getAcwardDetail,addAcward,updateAcward} from '@/api/entryaward/acaward.js'
  ...
  methods: {
    ...
    dataFormSubmit() {
      this.$refs["dataForm"].validate((valid) => {
        if (valid) {
            if(this.isadd==false){
                updateAcward(this.dataForm.id,this.dataForm).then(resp=>{
                    this.$message({
                    message: "修改成功",
                    type: "success",
                    duration: 1500,
                ...
                this.dataForm.verify=0
                addAcward(this.dataForm).then(resp=>{
                    this.$message({
                    message: "添加成功",
                    type: "success",
                    duration: 1500,
